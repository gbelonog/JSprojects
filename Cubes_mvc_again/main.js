...
class CubesModel{
    ...
    drow(size){
        this.cubesView.drowGrid(size);
    };
};
class CubesView{
    drowGrid(size) {
        let game = document.getElementById('board');//get board
        this.grid = document.createElement('grid');//create grid 
        let gridSize = size * 100 +size * 4;
...
let myGameController = new CubesController(myGameModel);
