...
//-------controller---------
var CubesController = function CubesController(cubesView, cubesModel) {
...
CubesController.getSizeFromUser = function getSizeFromUser(){
    this.cubesModel.size = prompt('Please set size of grid');
};

CubesController.onClickCube = function onClickCube(e) {
    var clicked = e.currentTarget;
    //var index = parseInt(target.dataset.penguinIndex, 10);
  
    //this.penguinModel.getPenguin(index, this.showPenguin.bind(this));

    this.cubesModel.cubesArray[clicked.id] = 1;
...
CubesController.getSizeFromUser();

function controller(event, param){
    switch(event){
        case 'start':
